...
public class ElyControlScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (this.gameObject.GetComponent<Rigidbody>().position.y < 1)
        {
        ...
        maxAbsForwardSpeed = newSpeed;
        float forwardAnimValue = Mathf.Clamp(Input.GetAxis("Vertical"), -maxAbsForwardSpeed, maxAbsForwardSpeed);
        anim.SetFloat("Speed", forwardAnimValue);
        if (Input.GetKey("e") || Input.GetKey("q"))
        {
            float turnValue = Mathf.Clamp(Input.GetAxis("Horizontal"), -1.0f / 2, 1.0f / 2);
            anim.SetFloat("Direction", turnValue);
        }
